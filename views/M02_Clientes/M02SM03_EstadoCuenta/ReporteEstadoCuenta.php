                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <center><label style="font-size: 18px; text-align: center; font-weight: bold;">ESTADO DE CUENTA</label></center>
                                <br>
                                ...
                                format(ROUND((SELECT (SUM(pagod.pagado) + if((select sum(monto_letra) from gp_cronograma where id_venta=gpv.id_venta AND dscto_acuerdo='1')>0,(select sum(monto_letra) from gp_cronograma where id_venta=gpv.id_venta AND dscto_acuerdo='1'),0)) FROM gp_pagos_detalle pagod, gp_pagos_cabecera pagoc, gp_cronograma gpcr 
                                WHERE pagod.idpago=pagoc.idpago AND pagoc.id_cronograma=gpcr.correlativo AND gpcr.id_venta=gpv.id_venta AND pagoc.id_venta=gpv.id_venta AND pagod.esta_borrado='0' AND pagoc.esta_borrado='0' AND pagod.id_venta=gpv.id_venta AND gpcr.estado=2 AND gpcr.esta_borrado='0'),1),2) as monto_pagado,
                                ...
                                format(((select sum(gpcr.monto_letra) as total from gp_cronograma gpcr WHERE gpcr.id_venta=gpv.id_venta AND gpcr.dscto_acuerdo='0' AND gpcr.esta_borrado='0')-(select sum(pagado) as total from gp_pagos_detalle WHERE id_venta=gpv.id_venta AND esta_borrado='0')),2) as monto_pendiente,
                                ...
                                format(((select sum(gpcr.monto_letra) as total from gp_cronograma gpcr WHERE gpcr.id_venta=gpv.id_venta AND gpcr.esta_borrado='0')-(select sum(pagado) as total from gp_pagos_cabecera WHERE id_venta=gpv.id_venta AND esta_borrado='0')),2) as letras_pendientes,
                                ...
                                WHERE dc.esta_borrado=0 AND gppc.esta_borrado='0' AND gppd.esta_borrado='0'
                    			AND dc.documento='$valor' $query_lote
                    			GROUP BY dc.id");
                            	...
                            	$ocultar_devolucion = 'style="display: none;"';
                            	if($devolucion=='1'){
                            	   $ocultar_devolucion = ""; 
                            	}else{
                            	   $ocultar_devolucion = 'style="display: none;"';
                                ...
                                $PrecioTotalLote = $respuesta_datos['importe_financiado'];
                                ...
                                <div class="table-responsive">
                                    <table style="width: 100%">
                                        <tr>
                                            <th style="background-color: #7BB1FF; color: white; width: 320px; font-size: 12px"><b><?php echo $dato; ?></b></th>
                                            <th style="background-color: #7BB1FF; color: white; width: 350px; font-size: 12px;margin-left: 1%"><b>UBICACIÓN : </b><?php echo $lote; ?></th>
                                        </tr>
                                    </table>
                                    <table style="width: 100%">
                                        <tr>
                                            <th style="font-size: 11px; width: 150px"><b>PRECIO DE VENTA : $ </b><?php echo $precio_pactado; ?></th>
                                            <th style="font-size: 11px; width: 220px"><b>INTERESES : $ </b><?php echo $interes; ?></th>
                                            <th style="font-size: 11px; width: 150px"><b>PRECIO TOTAL LOTE : $ </b><?php echo $PrecioTotalLote; ?></th>
                                        </tr>
                                        <tr>
                                            <th style="font-size: 11px; width: 150px"><b>CAPITAL VIVO : $ </b> <?php echo $monto_pendiente; ?></th>
                                            <th style="font-size: 11px; width: 220px"><b>TIEMPO FINANCIADO : </b><?php echo $TiempoFinanciado; ?></th>
                                            <th style="font-size: 11px; width: 150px"><b>TOTAL CANCELADO : $ </b><?php echo $monto_pagado; ?></th>
                                        </tr>
                                        <tr>
                                            <th style="width: 150px; font-size: 11px"><b>Correo : </b><?php echo $correo; ?></th>
                                            <th style="width: 220px; font-size: 11px"><b>Teléfono : </b><?php echo $contacto; ?></th>
                                            <th style="width: 150px; font-size: 11px"><b>Fecha Entrega Casa : </b><?php echo $fecha_entrega; ?></th>
                                        </tr>
                                    </table>
                                    <br>
                                </div>
                                <div class="col-md-12 text-center cancel" <?php echo $ocultar_cancel; ?>>C  A  N  C  E  L  A  D  O</div>
                                <div class="col-md-12 text-center cancel" <?php echo $ocultar_devolucion; ?>>D E V U E L T O</div>
                                <br>
                                    ...
                                    <div class="table-responsive">
                                        <?php
                                           ...
                                           $query_v = "";
                                            if(!empty($valor)){
                                                $query_lote	  = "";
                                                $query_documento = "AND dc.documento='$valor'"; 
                                                
                                                $consulta_idventa = mysqli_query($conection, "SELECT gpv.id_venta as id_venta FROM gp_venta gpv, datos_cliente dc WHERE gpv.id_cliente=dc.id AND dc.documento='$valor' AND gpv.esta_borrado='0'");
                                                $respuesta_idventa = mysqli_fetch_assoc($consulta_idventa);
                                                $idventa = $respuesta_idventa['id_venta'];
                                                $query_v = "AND gpcr.id_venta='$idventa'";
                                            ...
                                            $query_ventas = mysqli_query($conection,"SELECT
                                                ...
                                                (SELECT if((gppd.importe_pago)>0, gppd.nro_operacion,'Ninguno') FROM gp_pagos_cabecera gppc, gp_pagos_detalle gppd WHERE gppc.idpago=gppd.idpago AND gppc.id_cronograma=gpcr.correlativo AND gppc.id_venta=gpv.id_venta AND gppc.esta_borrado='0' AND gppd.esta_borrado='0' GROUP BY gppd.idpago) as nro_operacion,
                                                (SELECT concat(gppdc.serie,' - ',gppdc.numero)  FROM gp_pagos_cabecera gppc, gp_pagos_detalle gppd, gp_pagos_detalle_comprobante gppdc WHERE gppdc.idpago_detalle=gppd.idpago_detalle AND gppc.idpago=gppd.idpago AND gppc.id_cronograma=gpcr.correlativo AND gppc.id_venta=gpv.id_venta AND gppc.esta_borrado='0' AND gppd.esta_borrado='0' GROUP BY gppd.idpago) as boleta
										...
										<table class="table table-striped table-bordered" cellspacing="0" id="TablaHojaResumen" style="width: 100%; text-align: center;">
                                            <thead class="cabecera">
                                                <tr>
                                                    ...
                                                    <th style="font-size: 10px">Letra</th>
                                                    <th style="font-size: 10px">Monto</th>
                                                    <th style="font-size: 10px">Soles</th>
                                                    <th style="font-size: 10px">Dolares</th>
                                                    <th style="font-size: 10px">Estado</th>
													<th style="font-size: 10px">Mora</th>
                                                    <th style="font-size: 10px">Nro Operación</th>
                                                    <th style="font-size: 10px; width: 60px">Nro Boleta</th> 
                                                </tr>
                                            </thead>
                                            <tbody class="control-detalle">
                                                ...
                                                $boletas="";
                                                    ...
                                                    //consultar soles
                                                    $soles = "";
                                                    $consultar_soles = mysqli_query($conection, "SELECT if(SUM(gppd.importe_pago)>0,SUM(gppd.importe_pago),'0.00') AS soles 
                                                    FROM gp_pagos_cabecera gppc, gp_pagos_detalle gppd 
                                                    WHERE gppc.id_cronograma='$correlativo' 
                                                    AND gppc.id_venta='$idventa' 
                                                    AND gppc.idpago=gppd.idpago 
                                                    AND gppd.moneda_pago='15380'");
                                                    if($consultar_soles->num_rows > 0){
                                                        $respuesta = mysqli_fetch_assoc($consultar_soles);
                                                        $soles = $respuesta['soles'];
                                                    }else{
                                                        $soles = '0.00';
                                                    }
                                                    ...
                                                    //consultar dolares
                                                    $dolares = "";
                                                    $consultar_dolares = mysqli_query($conection, "SELECT if(SUM(gppd.importe_pago)>0,SUM(gppd.importe_pago),'0.00') AS dolares 
                                                    FROM gp_pagos_cabecera gppc, gp_pagos_detalle gppd 
                                                    WHERE gppc.id_cronograma='$correlativo' 
                                                    AND gppc.id_venta='$idventa' 
                                                    AND gppc.idpago=gppd.idpago 
                                                    AND gppd.moneda_pago='15381'");
                                                    if($consultar_dolares->num_rows > 0){
                                                        $respuesta = mysqli_fetch_assoc($consultar_dolares);
                                                        $dolares = $respuesta['dolares'];
                                                    }else{
                                                        $dolares = '0.00';
                                                    }
                                                ...
                                                <tr>
                                                    ...
                                                    <td <?php if($row['estado']=="1"){ ?> style="color: #FFA342" <?php }else{ if($row['estado']=="2"){ ?> style="color: #0EBF00" <?php }else{ ?> style="color: #DC0000" <?php }} ?>><?php echo $row['descEstado']; ?></td>
                                                    <td><?php echo $row['mora']; ?></td>
                                                    <td><?php echo $nro_operacion; ?></td>

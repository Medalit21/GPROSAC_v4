...
if(isset($_POST['btnListarTablaPagosComprobante'])){
    ...
    $query_ordenar="gppd.fecha_pago DESC";
        ...
        $query = mysqli_query($conection,"SELECT
            ...
            AND gppd.estado='2' and tipo_origen = '2'
            $query_proyecto
            $query_ec
            $query_ev
            $query_documento
			...
			$consultar_pago = mysqli_query($conection, "SELECT 
					...
					if ($result>0){
						...
						if(!empty($ComprobAdjVC)){
							...
							// Nueva consulta para traer el importe adicional
							$consulta_importe_2 = mysqli_query($conection, "SELECT
							gppdc.pagado as TotalImporte
							FROM gp_pagos_detalle_comprobante gppdc
							INNER JOIN gp_pagos_detalle AS gppd ON gppd.idpago_detalle=gppdc.idpago_detalle
							...
							$respuesta_importe_2 = mysqli_fetch_assoc($consulta_importe_2);
							$importe_2 = floatval(str_replace(',', '', $respuesta_importe_2['importe_2'])); // Limpiar y convertir el importe
							...
							if($debe_haber == "H"){
    							...
    							$insertar_pagoCabHab = mysqli_query($conection,"INSERT INTO ingresos_cabecera(Id_Cabecera, Sede, identificador, id_pago, Fecha, Moneda, TipoCambio, Glosa, Total, Cuenta_Contable, Operacion, Numero, Accion, flujo_caja, tipo_origen) 
    							VALUES ('$contador','$sede','$iddetalle','$idpago','$fecha_pago', '$moneda_pago','$tipo_cambio','$glosa','$importe_pago','$cuenta_contable', '$operacion','$numero','$accion','$flujo', '2')");

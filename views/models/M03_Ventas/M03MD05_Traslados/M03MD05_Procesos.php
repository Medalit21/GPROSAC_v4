...
if (isset($_POST['ReturnGuardarVenta'])) {
    ...
    $consultar_venta = mysqli_query($conection, "SELECT id_venta as id, id_lote as idlote FROM gp_venta WHERE id_venta='$__ID_VENTA' AND esta_borrado='0' AND estado='1'");
    $respuesta_venta = mysqli_num_rows($consultar_venta);

    if($respuesta_venta >0){
        
        $row = mysqli_fetch_assoc($consultar_venta);
        ...
        $insertar_venta = mysqli_query($conection, "INSERT INTO gp_traslado(id_cliente, id_lote, id_vendedor, descripcion, tipo_comprobante, serie, numero, fecha_venta, tipo_inmobiliaria, tipo_casa, fecha_entrega_casa, condicion, tipo_moneda, tipo_cambio, total, cantidad_letra, tna, primera_fecha, tiene_cuota_inicial, monto_cuota_inicial,id_usuario_crea, id_lote_ref, venta_traslado) VALUES 
        ('$id_cliente','$cbxLoteT','$idUsuario','TRASLADO DE LOTE','$tipo_comprobante','$serie','$numero','$fecha','$cbxTipoInmueble','$cbxTipoCasa','$txtFechaEntregaCasa','$cbxCondicion','$cbxTipoMonedaVenta','$txtTipoCambioT','$total_venta','$txtCantidadLetras','$txtNuevaTEA','$txtPrimeraFecha','1','$total_inicial','$idUsuario','$idlote','$__ID_VENTA')");

        //ACTUALIZAR VENTA
        $actualiza_venta = mysqli_query($conection, "UPDATE gp_venta SET id_lote='$cbxLoteT', id_traslado='1' WHERE id_venta='$__ID_VENTA'");
        
        //ACTUALIZAR LOTE ANTIGUO
        $actualiza_estado_lote = mysqli_query($conection, "UPDATE gp_lote SET estado='1' WHERE idlote='$idlote'");

        //ACTUALIZAR LOTE NUEVO
        $actualiza_estado_lote = mysqli_query($conection, "UPDATE gp_lote SET estado='6' WHERE idlote='$cbxLoteT'");

        
        $data['status'] = 'ok';
        $data['data'] = 'Se ha generado el traslado del lote para el registro seleccionado. Se sugiere revisar la Hoja Resumen para validar el traslado para el propietario.';

...
if (isset($_POST['ReturnGuardarVenta'])) {
    ...
    $cbxLoteT = isset($_POST['cbxLoteT']) ? $_POST['cbxLoteT'] : Null;
    $cbxLoteT = trim($cbxLoteT);

    $cbxTipoInmueble = isset($_POST['cbxTipoInmueble']) ? $_POST['cbxTipoInmueble'] : Null;
    $cbxTipoInmueble = trim($cbxTipoInmueble);

    $cbxTipoCasa = isset($_POST['cbxTipoCasa']) ? $_POST['cbxTipoCasa'] : Null;
    $cbxTipoCasa = trim($cbxTipoCasa);

    $cbxTipoMonedaVenta = isset($_POST['cbxTipoMonedaVenta']) ? $_POST['cbxTipoMonedaVenta'] : Null;
    $cbxTipoMonedaVenta = trim($cbxTipoMonedaVenta);

    $txtTipoCambioT = isset($_POST['txtTipoCambioT']) ? $_POST['txtTipoCambioT'] : Null;
    $txtTipoCambioT = trim($txtTipoCambioT);
    ...
    $txtPrimeraFecha = isset($_POST['txtPrimeraFecha']) ? $_POST['txtPrimeraFecha'] : Null;
    $txtPrimeraFecha = trim($txtPrimeraFecha);

    $txtNuevaTEA = isset($_POST['txtNuevaTEA']) ? $_POST['txtNuevaTEA'] : Null;
    $txtNuevaTEA = trim($txtNuevaTEA);

    $fileContrato = isset($_POST['fileContrato']) ? $_POST['fileContrato'] : Null;
    $fileContrato = trim($fileContrato);
    ...
    $cbxCondicion = isset($_POST['cbxCondicion']) ? $_POST['cbxCondicion'] : Null;
    $cbxCondicion = trim($cbxCondicion);

    $__ID_VENTA = isset($_POST['__ID_VENTA']) ? $_POST['__ID_VENTA'] : Null;
    $__ID_VENTA = trim($__ID_VENTA);

    //CONSULTAR IDUSUARIO
    $consultar_idusuario = mysqli_query($conection, "SELECT idusuario as id FROM persona WHERE DNI='$ValidUsuario'");
    $respuesta_idusuario = mysqli_fetch_assoc($consultar_idusuario);
    $idUsuario = $respuesta_idusuario['id'];


    //CONSULTAR EL REGISTRO DE VENTA
    $consultar_venta = mysqli_query($conection, "SELECT id_venta FROM gp_venta WHERE id_venta='$__ID_VENTA' AND esta_borrado='0' AND estado='1'");
    $respuesta_venta = mysqli_num_rows($consultar_venta);

    if($respuesta_venta >0){

        //CONSULTAR DATOS DE VENTA ANTERIOR
        $consultar_datos_venta = mysqli_query($conection,"SELECT
            gpv.id_cliente as idcliente,
            gpv.tipo_comprobante as tipoComprobante,
            gpv.serie as serie,
            gpv.numero as numero,
            gpv.total as total,
            (select SUM(importe_pago) from gp_pagos_detalle WHERE id_venta=gpv.id_venta AND estado='2' AND esta_borrado='0') as total_pagado
            FROM gp_venta gpv WHERE gpv.id_venta='$__ID_VENTA'");
        $respuesta_datos_venta = mysqli_fetch_assoc($consultar_datos_venta);

        $id_cliente = $respuesta_datos_venta['idcliente'];
        $tipo_comprobante = $respuesta_datos_venta['tipoComprobante'];
        $serie = $respuesta_datos_venta['serie'];
        $numero = $respuesta_datos_venta['numero'];
        $total_venta = $respuesta_datos_venta['total'];
        $total_pagado = $respuesta_datos_venta['total_pagado'];

        $total_inicial = $total_pagado + $txtNuevoMontoInicial;

        //GRABAR NUEVA VENTA - TRASLADO
        $insertar_venta = mysqli_query($conection, "INSERT INTO gp_venta(idcliente, id_lote, id_vendedor, descripcion, tipo_comprobante, serie, numero, fecha_venta, tipo_inmobiliaria, tipo_casa, fecha_entrega_casa, condicion, tipo_moneda, tipo_cambio, total, cantidad_letra, tna, primera_fecha, tiene_cuota_inicial, monto_cuota_inicial,id_usuario_crea, id_traslado, venta_traslado) VALUES (
            '$id_cliente',
            '$cbxLoteT',
            '$idUsuario',
            'TRASLADO DE LOTE',
        ...
        if($insertar_venta){
            ...
            //Actualizar Estado de cronograma anterior
            $actualizar_cronograma = mysqli_query($conection, "UPDATE gp_cronograma SET esta_borrado='1', borrado='$control', id_usuario_borra='$idUsuario' WHERE id_venta='$__ID_VENTA'");

            //VERIFICAR CONTEO DE PAGOS CABECERA
            $consultar_conteo_cab_1=mysqli_query($conection, "SELECT idpago FROM gp_pagos_cabecera WHERE id_venta='$__ID_VENTA' AND esta_borrado='0' AND estado='2'");
            $respuesta_conteo_cab_1=mysqli_num_rows($consultar_conteo_cab_1);

            $consultar_conteo_cab_2=mysqli_query($conection, "SELECT idpago FROM gp_pagos_cabecera WHERE id_venta='$__ID_VENTA' AND esta_borrado='0' AND visto_bueno='2' AND estado='2'");
            $respuesta_conteo_cab_2=mysqli_num_rows($consultar_conteo_cab_2);

            $conteo_cab_1 = $respuesta_conteo_cab_1;
            $conteo_cab_2 = $respuesta_conteo_cab_2;

            //VERIFICAR CONTEO DE PAGOS DETALLE
            $consultar_conteo_det_1=mysqli_query($conection, "SELECT idpago FROM gp_pagos_detalle WHERE id_venta='$__ID_VENTA' AND esta_borrado='0'");
            $respuesta_conteo_det_1=mysqli_num_rows($consultar_conteo_det_1);

            $consultar_conteo_det_2=mysqli_query($conection, "SELECT idpago FROM gp_pagos_detalle WHERE id_venta='$__ID_VENTA' AND esta_borrado='0' AND estado='2'");
            $respuesta_conteo_det_2=mysqli_num_rows($consultar_conteo_det_2);

            $conteo_det_1 = $respuesta_conteo_det_1;
            $conteo_det_2 = $respuesta_conteo_det_2;

            if(($conteo_cab_1==$conteo_cab_2) && ($conteo_det_1==$conteo_det_2)){

                $consultar_id_venta_nueva = mysqli_query($conection, "SELECT id_venta as id FROM gp_venta WHERE venta_traslado='$__ID_VENTA'");
                $respuesta_id_venta_nueva = mysqli_fetch_assoc($consultar_id_venta_nueva);
                $id_venta_nueva = $respuesta_id_venta_nueva['id'];

                //Actualizar pagos cabecera
                $actualizar_pagos_detalle = mysqli_query($conection, "UPDATE gp_pagos_detalle SET id_venta='$id_venta_nueva', idpago='1', actualizado='$control', id_usuario_actualiza='$idUsuario' WHERE id_venta='$__ID_VENTA'");

                $actualizar_pagos_cabecera = mysqli_query($conection, "UPDATE gp_pagos_cabecera SET id_venta='$id_venta_nueva', idpago='1', actualizado='$control', id_usuario_actualiza='$idUsuario' WHERE id_venta='$__ID_VENTA'");
                ...
                $txtTEAr = $txtNuevaTEA;
                $txtCuotasr = $txtCantidadLetras;
                $txtPrecioVentar = $total_venta;
                $txtCuotaInicialr = $txtNuevoMontoInicial + $total_pagado;

                $txtPrecioVentar = $txtPrecioVentar - $txtCuotaInicialr;
               ...
               //fechas de pago
               $fecha_pago_cuota = $txtFechaInicio;
                ...
                for($cont=0; $cont<=$txtCuotasr; $cont++){
                    ...
                    if($cont==0 || $cont==1){
                        ...
                        $capital_1 = $total_venta - $total_pagado;
                        $capital_2 = $capital_1 - $txtNuevoMontoInicial;

                        $insertar_fila1 = mysqli_query($conection,"INSERT INTO gp_cronograma(id_venta, item_letra, correlativo, fecha_vencimiento, monto_letra, interes_amortizado, capital_amortizado, capital_vivo, estado, id_usuario_crea) VALUES
                        ('$IDVENTA','C.I.','1','$fecha', '$total_pagado', '0.00', '0.00', '$capital_1', '1', '$idUsuario')");

                        $insertar_fila2 = mysqli_query($conection,"INSERT INTO gp_cronograma(id_venta, item_letra, correlativo, fecha_vencimiento, monto_letra, interes_amortizado, capital_amortizado, capital_vivo, estado, id_usuario_crea) VALUES
                        ('$IDVENTA','C.I.','2','$fecha_pago_cuota', '$txtCuotaInicialr', '0.00', '0.00', '$capital_2', '1', '$idUsuario')");
                        ...
                        $c = $cont + 1;
                         $insertar_fila1 = mysqli_query($conection,"INSERT INTO gp_cronograma(id_venta, item_letra, correlativo, fecha_vencimiento, monto_letra, interes_amortizado, capital_amortizado, capital_vivo, estado, id_usuario_crea) VALUES
                        ('$IDVENTA','$cont','$c','$fecha_pago_cuota', '$cuota', '$intereses', '$capital_amortizado', '$capital_vivo', '1', '$idUsuario')");
                    }
                }

                //ACTUALIZAR ESTADO DEL REGISTRO DE VENTA ACTUAL
                $actualizar_estado = mysqli_query($conection, "UPDATE gp_venta SET esta_borrado='1', estado='0', id_usuario_actualiza='$idUsuario', actualizado='$control' WHERE id_venta='$__ID_VENTA'");

                $data['status'] = 'ok';
                $data['data'] = 'Se ha generado la amortizacion para el registro de venta seleccionado.';

            }else{

              $data['status'] = 'bad';
              $data['data'] = 'Verificar los pagos registrados para la venta. Recuerde que para poder realizar el traslado de la venta es necesario que todos los pagos realizados para la venta, aun vigente, hayan sido validados.';
            ...
            $data['status'] = 'bad';
            $data['data'] = 'El traslado no pudo ser registrado. Revise los datos e intente nuevamente.';
        }

    }else{

        $data['status'] = 'bad';
        $data['data'] = 'Seleccione un  registro de venta valido sobre el cual aplicar el traslado.';
...
if (isset($_POST['ValidarTipoMoneda'])) {
    ...
    $consultar_tipocambio = mysqli_query($conection, "SELECT COUNT(idconfig_tipo_cambio) as total, SUM(valor) as suma FROM configuracion_tipo_cambio");
    $respuesta_tipocambio = mysqli_fetch_assoc($consultar_tipocambio);
    $cantidad = $respuesta_tipocambio['total'];
    $suma =  $respuesta_tipocambio['suma'];

    $tipo_cambio = $suma / $cantidad;
...
if (isset($_POST['btnBuscarImportesVentas'])) {
    ...
    if($query->num_rows > 0) {
        ...
        $data['idtraslado'] = $respuesta['idtraslado'];
...
if (isset($_POST['btnCalcularTotalInicial'])) {
    ...
    $consultar_total_venta = mysqli_query($conection, "SELECT total as total FROM gp_venta WHERE id_venta='$idVenta'");
    $respuesta_total_venta = mysqli_fetch_assoc($consultar_total_venta);
    $total_venta = $respuesta_total_venta['total'];

    $consultar_totalLetras = mysqli_query($conection,"SELECT format(($total_venta - (SUM(importe_pago) + '$montoInicial')),2) as total FROM gp_pagos_detalle WHERE id_venta='$idVenta' AND estado='2'");
    $respuesta_totalLetras = mysqli_fetch_assoc($consultar_totalLetras);
...
if (isset($_POST['btnGenerarDatosVentasTraslado'])) {
    ...
    $consultar_datos_actual = mysqli_query($conection, "SELECT
        gpv.id_lote as id,
        gpl.valor_con_casa as precio_con_casa,
        gpl.valor_sin_Casa as precio_sin_casa,
        gpv.tipo_casa  as tipo_casa,
        (select SUM(importe_pago) from gp_pagos_detalle WHERE id_venta=gpv.id_venta AND estado='2') as total_pagado,
        format((select SUM(importe_pago) from gp_pagos_detalle WHERE id_venta=gpv.id_venta AND estado='2'),2) as total_pagado_formato,
        gpv.tipo_moneda as tipo_moneda
        FROM gp_venta gpv
        INNER JOIN gp_lote AS gpl ON gpl.idlote=gpv.id_lote
        WHERE gpv.id_venta='$idVenta'");
    $respuesta_datos_actual = mysqli_fetch_assoc($consultar_datos_actual);

    $tipo_Casa="";

    $precio_con_casa = $respuesta_datos_actual['precio_con_casa'];
    $precio_sin_casa = $respuesta_datos_actual['precio_sin_casa'];
    $tipo_Casa = $respuesta_datos_actual['tipo_casa'];
    $total_pagado = $respuesta_datos_actual['total_pagado'];
    $total_pagado_formato = $respuesta_datos_actual['total_pagado_formato'];
    $tipo_moneda = $respuesta_datos_actual['tipo_moneda'];
    $precio_inicio = 0;

    $consultar_datos_nuevo = mysqli_query($conection, "SELECT format(valor_con_casa,2) as precio_con_casa, format(valor_sin_casa,2) as precio_sin_casa, format((valor_con_casa - $total_pagado),2) as financiado_con_casa, format((valor_sin_casa - $total_pagado),2) as financiado_sin_casa FROM gp_lote WHERE idlote='$idlote'");
    $respuesta_datos_nuevo = mysqli_fetch_assoc($consultar_datos_nuevo);

    $precio_con_casa_nuevo = $respuesta_datos_nuevo['precio_con_casa'];
    $precio_sin_casa_nuevo = $respuesta_datos_nuevo['precio_sin_casa'];
    $financiado_con_casa = $respuesta_datos_nuevo['financiado_con_casa'];
    $financiado_sin_casa = $respuesta_datos_nuevo['financiado_sin_casa'];
    $precio_nuevo = 0 ;

    if($propiedad == 1){
        $precio_inicio = $precio_con_casa;
        $precio_nuevo = $precio_con_casa_nuevo;
        $precio_financiado = $financiado_con_casa;
    }else{
        $precio_inicio = $precio_sin_casa;
        $precio_nuevo = $precio_sin_casa_nuevo;
        $precio_financiado = $financiado_sin_casa;
    }


    if($precio_nuevo >= $precio_inicio){

        //Datos
        $PRECIO_VENTA = $precio_nuevo;
        $CAPITAL_INICIAL = $total_pagado_formato;
        $TOTAL_FINANCIAR = $precio_financiado;
    }else{
    ...
    if ($consultar_total_letras->num_rows > 0) {
        ...
        $data['total_inicial'] = $TOTAL_FINANCIAR;
...
/*******************BUSCAR INFORMACION LOTE************************** */
if (isset($_POST['btnLlenarDatosCronograma'])) {
    
    $idVenta = $_POST['idVenta'];

    $query = mysqli_query($conection, "SELECT
    format(SUM(gppd.pagado),2) as pagado,
    (select count(id) from gp_cronograma where id_venta='$idVenta' AND estado='2' AND pago_cubierto in (1,2)) as tot_letras
    FROM gp_pagos_detalle gppd
    INNER JOIN gp_pagos_cabecera AS gppc ON gppc.idpago=gppd.idpago
    INNER JOIN gp_cronograma AS gpcro ON gpcro.correlativo=gppc.id_cronograma AND gpcro.id_venta=gppc.id_venta
    WHERE gppd.id_venta='$idVenta' AND gpcro.estado='2' AND gpcro.pago_cubierto in (1,2)");
...
if (isset($_POST['btnGuardarRefinanciamientoVenta'])) {
    
    $txtNuevasLetras = $_POST['txtNuevasLetras'];
    $txtTNA = $_POST['txtTNA'];
    $idVenta = $_POST['__ID_VENTA'];

    //Eliminar las cuotas pendientes por pagar
    $eliminar = mysqli_query($conection, "DELETE FROM gp_cronograma WHERE id_venta='$idVenta' AND estado in (1,3)");
    
    //obtener el nuevo capital vivo 
        //precio de venta
        $consultar_precio = mysqli_query($conection, "SELECT total as total FROM gp_venta WHERE id_venta='$idVenta'");
        $respuesta_precio = mysqli_fetch_assoc($consultar_precio);
        $total_venta = $respuesta_precio['total'];
        
        //total pagado
        $consultar_pagado = mysqli_query($conection, "SELECT sum(pagado) as total FROM gp_pagos_detalle WHERE id_venta='$idVenta'");
        $respuesta_pagado = mysqli_fetch_assoc($consultar_pagado);
        $total_pagado = $respuesta_pagado['total'];
        
        $nuevo_capital = $total_venta - $total_pagado;
        
    //Calcular la cuota mensual
        //obteniendo la tasa mensual
        $valor_TEA= $txtTNA/100;
	    $valor_TEM = pow(( 1 + $valor_TEA),(30/360)) - 1;
	    
	    $txtCuotasr = $txtNuevasLetras;
	    $capital_vivo = $nuevo_capital;
	    
	    //CALCULO CUOTA MENSUAL
	    $valor_cuota = (($capital_vivo * ($valor_TEM * pow((1 + $valor_TEM),$txtCuotasr)))/((pow((1+$valor_TEM),$txtCuotasr))-1));
	    
	 //obtener el item letra y correlativo maximo
	 $consultar_max = mysqli_query($conection, "SELECT max(item_letra) as letra, max(correlativo) as correlativo, max(fecha_vencimiento) as fecha FROM gp_cronograma WHERE id_venta='$idVenta'");
	 $respuesta_max = mysqli_fetch_assoc($consultar_max);
	 $max_letras = $respuesta_max['letra'];
	 $max_letras = $max_letras + 1;
	 $max_correlativo = $respuesta_max['correlativo'];
     $max_correlativo = $max_correlativo + 1;
     
     $max_fecha = $respuesta_max['fecha'];
    ...
    for($cont=0; $cont<=$txtCuotasr; $cont++){
        ...
        $capital_amortizado = $amortizacion;
        $total_pagado = $cuota;
        $fecha_pago_cuota = date("Y-m-d",strtotime($max_fecha."+ 1 month"));
        
        $insertar_fila0 = mysqli_query($conection,"INSERT INTO gp_cronograma(id_venta, item_letra, correlativo, fecha_vencimiento, monto_letra, interes_amortizado, capital_amortizado, capital_vivo, estado, es_cuota_inicial, pago_cubierto) VALUES
        ('$idVenta','$max_letras','$max_correlativo','$fecha_pago_cuota','$cuota','$intereses','$capital_amortizado','$capital_vivo','1','0','0')");
    ...
    if ($insertar_fila0) {
        ...
        $data['status'] = 'ok';
        $data['data'] = 'Se ha refinanciado la venta.';
        
    } else {
        $data['status'] = 'bad';
        $data['data'] = 'No se completo el proceso. Intente nuevamente.';
    }
    header('Content-type: text/javascript');
    echo json_encode($data, JSON_PRETTY_PRINT);
}

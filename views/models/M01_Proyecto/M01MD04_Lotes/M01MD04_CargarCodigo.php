...
if (!empty($_POST)) {
    if (isset($_POST['btnCargarCodigoLt'])) {
       
        $CorreProyecto="";
        $SiglaProyecto ="";

        $bxManzanaslte = isset($_POST['bxManzanaslte']) ? $_POST['bxManzanaslte'] : Null;
        $cbxManzanasltr = trim($bxManzanaslte); 

        $consultar_Correproyecto = mysqli_query($conection, "SELECT max(correlativo) as correlativo FROM gp_lote WHERE estado='1' AND idmanzana='$cbxManzanasltr'");
        ...
        $query = mysqli_query($conection, "SELECT nombre_corto as sigla FROM configuracion_detalle WHERE codigo_tabla='_SIGLA' AND nombre_largo='LOTE'");
